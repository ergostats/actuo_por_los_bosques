...
aread_protegidas <- read_sf("data/hc000_bosque_vegetacion_protectora/hc000_bosque_vegetacion_protectora.shp")
...
Origenes y destinos:
>>>>>>> 0e57e3260c23b86cf71e0c9d974fab28b85008f7
...
aread_protegidas <- st_transform(aread_protegidas, "+proj=merc")

shape <- st_transform(shape, "+proj=merc")

# shape <- sf::st_simplify(shape)
...
```{r}
tabla_plot_aprovechamiento <- shape %>% 
...
tabla_plot_aprovechamiento %>% 
  ...
  geom_point(data = tabla_aprovechamiento, 
             aes(x = x,
                 y = y)) +
